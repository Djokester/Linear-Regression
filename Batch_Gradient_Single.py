...
f = open('/home/djokester/GitHub/mlsn/ex1data1.txt').read()
f = f.split()
x = []
y = []
for line in f:
...
#Creating a new set to apply our regression model
xdash = list(np.linspace(5,25,100))
#increasing the number of 1s from 97 to 100
for i in range(0,3):
    xo.append(1)
Xdash = np.matrix([xo, xdash])
YDash = np.dot(theta_dash,Xdash)
#Plotting the Training set and the Regression Model
plt.plot(np.matrix(xdash) ,YDash, 'bo')
plt.plot(x, y, 'ro')
plt.show()